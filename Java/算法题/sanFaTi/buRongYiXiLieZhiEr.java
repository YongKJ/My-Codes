...
public class buRongYiXiLieZhiEr {
	
	public int sheeps(int n) {
		if(n==0) {
			return 3;
		}else {
			return (sheeps(n-1)-1)*2;
		}
	}
	...
	public static void main(String[] args) {
		...
		while(n-->0) {
			int m = sc.nextInt();
			buRongYiXiLieZhiEr b = new buRongYiXiLieZhiEr();
			System.out.println(b.sheeps(m));
		}
		sc.close();
	}
...
重庆市郊黄泥板村的徐老汉（大号徐东海，简称XDH）这两年辛辛苦苦养了不少羊，到了今年夏天，由于众所周知的高温干旱，
实在没办法解决牲畜的饮水问题，就决定把这些羊都赶到集市去卖。从黄泥板村到交易地点要经过N个收费站，按说这收费站和
徐老汉没什么关系，但是事实却令徐老汉欲哭无泪：
...
由于徐老汉没钱，收费员就将他的羊拿走一半，看到老汉泪水涟涟，犹豫了一下，又还给老汉一只。巧合的是，后面每过一个收
费站，都是拿走当时羊的一半，然后退还一只，等到老汉到达市场，就只剩下3只羊了。
