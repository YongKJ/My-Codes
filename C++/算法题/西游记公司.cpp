...
#include <string>
using namespace std;
long time(int x, int y, int z) { return x * 60 * 60 + y * 60 + z; }
int num(string a, int &i) {
  int x;
  if (a[i + 1] != ':')
    x = (a[i] - '0') * 10 + a[i + 1] - '0', i += 2;
  else
    x = a[i] - '0', i++;
  return x;
}
long judge(string a, string b) {
  int x, y, z, x1, y1, z1,flag=0;
  ...
  return time(x1, y1, z1) - time(x, y, z);
}
int main() {
  int n, t;
  string a, b;
